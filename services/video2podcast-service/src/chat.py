...
class Chat:
    ...
    def create_chat(self, text_path: str, context: str, person1: str = 'Liz', person2: str = 'John', filename: str = 'chat.txt', max_tokens=100000) -> str:
        ...
        if not os.path.exists(output_path):
            ...
            response = client.chat.completions.create(   
                model='gpt-4o', 
                messages=[{'role': 'user', 'content': f"Given that you are an expert on {context}, create a laid-back podcast conversation between two people: {person1} (main host) and {person2} (co-host). They know each other for years and constantly make fun of each other. The podcast's name you have to create yourself. The conversation can be really long, with both talking like in a casual podcast and must end with the hosts closing the podcast. In the middle of the conversation, the host have to annouce an ad about a fictitional product, related to the topic, that you have to make up. Right before the end, they have to read messages from listeners and you have to make up the listeners names and messages. Please don't write any titles or lines that are markups for the script, leave only the lines for each person talking. Also, space each persons line with an empty line and before each persons line write their name like the following pattern: Person1: <his/her line>. They are talking about the following topics:\n\n"+text}], 
                max_tokens=max_tokens,
            )
